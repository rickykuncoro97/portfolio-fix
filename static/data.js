export const menukiri = {
  img_user: "https://raw.githubusercontent.com/rickykuncoro97/myportfolio.io/main/pp.jpg",
  nama: "Ricky Kuncoro",
  profesi: "Front End Developer",
  umur: "23 tahun",
  alamat: "Delta Sari Indah, Waru",
  hp: "087751132069",
  status: {
    name: "Sedang Bekerja",
    st: 1
    // name: "Siap Bekerja",
    // st: 0
  },
  bahasa: [{
      name: "Indonesia",
      value: 100
    },
    {
      name: "Inggris",
      value: 50
    },
    {
      name: "Jawa",
      value: 70
    },
  ],
  skills: [{
      name: "HTML",
      value: 80
    },
    {
      name: "CSS",
      value: 70
    },
    {
      name: "JS",
      value: 80
    },
    {
      name: "Vue",
      value: 80
    },
    {
      name: "Nuxt JS",
      value: 70
    },
    {
      name: "PHP",
      value: 50
    },
  ],
  soft_skills: [{
      name: "Komunikasi",
      value: 50
    },
    {
      name: "Berpikir Kritis",
      value: 70
    },
    {
      name: "Kepemimpinan",
      value: 30
    },
    {
      name: "Perilaku Baik",
      value: 90
    },
    {
      name: "Kerja Tim",
      value: 90
    },
    {
      name: "Etos Kerja",
      value: 70
    },
  ],
  cv: "https://google.com"
}

export const menukanan = {
  bannerperkenalan: {
    nama: "Saya Ricky Kuncoro",
    img_user: "https://raw.githubusercontent.com/rickykuncoro97/myportfolio.io/main/pp.jpg",
    pekerjaan: "Front-End Developer ",
    deskripsi: "Saya sudah bekerja sebagai seorang front-end developer selama 1 tahun disebuah software house. Sudah banyak proyek yang saya kerjakan dan mayoritas menggunakan framework Vue & Nuxt.Js",
    btn: {
      class: "col-5 mt-5",
      value: "HUBUNGI SAYA"
    }
  },
  pendidikan: {
    judul: "Pendidikan",
    details: [{
      judulkiri: "Universitas Kristen Petra",
      judulkanan: "S1 Teknik Informatika",
      deskripsikiri: "Mahasiswa",
      sampai: "2016 - 2020",
      deskripsikanan: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Amet dapibus nibh ut faucibus nunc, egestas id amet porttitor. Pulvinar quisque sed amet, nulla nunc. Eleifend sodales posuere fusce tempus etiam et pellentesque. Molestie risus enim neque eget dui"
    },{
        judulkiri: "SMK Kristen Petra",
        judulkanan: "Jurusan Teknik Komputer & Jaringan",
        deskripsikiri: "Siswa",
        sampai: "2013 - 2016",
        deskripsikanan: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Amet dapibus nibh ut faucibus nunc, egestas id amet porttitor. Pulvinar quisque sed amet, nulla nunc. Eleifend sodales posuere fusce tempus etiam et pellentesque. Molestie risus enim neque eget dui"
      }]
  },
  pekerjaan: {
    judul: "Pekerjaan",
    details: [{
      judulkiri: "PT. DINAMIKA INTEGRA",
      judulkanan: "Web Developer",
      deskripsikiri: "Magang",
      sampai: "Jul 2019 - Okt 2019",
      deskripsikanan: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Amet dapibus nibh ut faucibus nunc, egestas id amet porttitor. Pulvinar quisque sed amet, nulla nunc. Eleifend sodales posuere fusce tempus etiam et pellentesque. Molestie risus enim neque eget dui"
    },{
        judulkiri: "PT. L'VIORS JAYA SENTOSA",
        judulkanan: "Staff IT",
        deskripsikiri: "Fulltime",
        sampai: "Des 2020 - Feb 2021",
        deskripsikanan: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Amet dapibus nibh ut faucibus nunc, egestas id amet porttitor. Pulvinar quisque sed amet, nulla nunc. Eleifend sodales posuere fusce tempus etiam et pellentesque. Molestie risus enim neque eget dui"
      },
      {
        judulkiri: "PT. DIGITAL ASIA SOLUSINDO",
        judulkanan: "Front-End Developer",
        deskripsikiri: "Fulltime",
        sampai: "Mei 2021 - Sekarang",
        deskripsikanan: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Amet dapibus nibh ut faucibus nunc, egestas id amet porttitor. Pulvinar quisque sed amet, nulla nunc. Eleifend sodales posuere fusce tempus etiam et pellentesque. Molestie risus enim neque eget dui"
      }]
  },
}
